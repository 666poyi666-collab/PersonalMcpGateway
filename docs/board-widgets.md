...
浏览器看板底部的「扩展面板」是留给你自己的区域。MCP 项目状态只是第一批内容；日程、
项目进度、新闻、股票、专注统计这类东西都通过这里接进来。带有内置项目分组的数据还会
供应对应的桌面独立卡片；普通自定义模块不再额外占用桌面管理页的常驻页签。
...
没有这个文件时，浏览器扩展面板会显示一张「如何开始」的说明卡片——这不是错误。
...
保存后等一次刷新（4 秒），模块会出现在浏览器扩展面板；属于内置项目分组的模块也会
更新对应桌面卡片。
...
只做只读的 `git` 查询，不会改动任何仓库；每个路径必须指向 Git 仓库根目录，结果缓存
60 秒。路径不存在、只是某个仓库里的普通子目录、或不是 Git
